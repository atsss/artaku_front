  ...
  <>
    {processes.map((pro) => (
      <Section key={pro.id} className="mt-16" isGray>
        ...
        {pro.summaryVideoId && <Youtube id={pro.summaryVideoId} className="mt-4"/>}
        {pro.descriptions &&
        ...
        {pro.imageUrls && pro.imageUrls.map((imageUrl, index) => (
          <div key={index} className="mt-8">
            <Image src={imageUrl} width={800} height={500} />
          </div>
        ))}
        {pro.videoIds && pro.videoIds.map((videoId, index) => (
          <div key={index} className="mt-8">
            <Youtube id={videoId}/>
          </div>
        ))}
